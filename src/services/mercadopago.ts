...
function priceFromPlan(plan: "pro_month" | "pro_year") {
  if (plan === "pro_year") {
    return {
      amount: 299.9,
      freq: 12,
...
export async function mpCreateSubscription(
  ...
  const { amount, freq, label } = priceFromPlan(plan);

  const site = getSiteUrl();

  const notificationUrl =
    `${site}/api/mp/webhook?secret=${encodeURIComponent(webhookSecret)}`;

  const backUrl = `${site}/checkout`;
