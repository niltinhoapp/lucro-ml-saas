...
type TooltipLabel = "Receita" | "Despesas" | "Lucro";
...
export default function GraficoLucro({ resultado, destaque }: Props) {
  const data = useMemo(
    () => [
      {
        modelo: "FULL",
  ...
  const melhor = resultado?.melhor ?? "FULL";
  const diff = Number(resultado?.diferencaLucro ?? 0);
  const diffAbs = Math.abs(diff);
  const diffSide: "FULL" | "FLEX" = diff >= 0 ? "FULL" : "FLEX";
    ...
    <section className="card">
      <div className="card-head">
        <div>
          <h2>Comparação</h2>
          <p>
            Melhor: <b>{melhor}</b> • Diferença: <b>{moeda(diffAbs)}</b> •
        ...
        >
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <YAxis
                tick={{ fill: "rgba(229,231,235,.70)", fontSize: 12 }}
              />
