    ...
    <div className="rank-block">
      ...
      <div className="rank-block-body">
        {items.length ? (
          ...
          <div className="rank-empty">Nenhum dado encontrado nessa leitura.</div>
        )}
      </div>
    </div>
  );
...
export default function ProdutosRank({ linhas }: { linhas?: LinhaVenda[] | null }) {
  ...
  async function run() {
    ...
    try {
      if (!linhasOk.length) {
        setErr(
          "Este relatório ainda não trouxe as linhas detalhadas por produto. Para gerar o ranking, a simulação precisa retornar esse nível de detalhe."
        );
        return;
      }
  ...
  return (
    <section className="ai-card rank-card">
      <div className="ai-head">
        <div>
          <h3 className="ai-title">Leitura dos produtos da operação</h3>
          <p className="ai-desc">
            Descubra quais produtos mais ajudam no lucro, quais geram prejuízo e quais estão pesando mais em taxa e logística.
          </p>
        </div>
      ...
      <div className="ai-body">
        <div className="ai-actions">
          <button onClick={run} disabled={loading} className="btn btn-primary">
            {loading ? "Analisando produtos..." : "Gerar ranking"}
          </button>
        </div>

        {err && (
          <div className="ai-result rank-error">
            <h4>Não foi possível concluir a leitura</h4>
            <p>{err}</p>
          </div>
        )}
        ...
        {data && (
          <>
            <div className="rank-summary">
              ...
              <div className="rank-summary-box">
                <div className="rank-summary-k">Itens por ranking</div>
                <div className="rank-summary-v">{data.top_n}</div>
              </div>
            </div>

            <div className="rank-grid">
              <RankBlock title="Produtos com mais lucro" icon="🏆" items={data.top_lucro} tone="good" />
              <RankBlock title="Produtos com mais prejuízo" icon="🧨" items={data.top_prejuizo} tone="bad" />
              <RankBlock title="Produtos com taxa mais pesada" icon="💸" items={data.top_taxa} tone="blue" />
              <RankBlock title="Produtos com logística mais pesada" icon="🚚" items={data.top_logistica} tone="neutral" />
            </div>
          </>
        )}
        ...
        {!data && !err && (
          <div className="ai-result">
            <h4>Gerar leitura dos produtos</h4>
            <p>
              Clique em <strong>Gerar ranking</strong> para entender quais produtos
              mais ajudam no resultado, quais estão puxando prejuízo e onde sua operação
              está sendo mais pressionada.
            </p>
          </div>
        )}
      </div>
